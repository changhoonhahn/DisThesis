...
connection. \todo{more summarizing remarks}
...
%\textit{In\ my\ research\ program, I will build on frameworks I have implemented for BOSS and tackle key methodological challenges in galaxy clustering with robust treatment of systematics, innovative approaches to modeling, and higher order statistics.  By unlocking the full potential of eBOSS and DESI, I will test General Relativity (GR) and constrain the total neutrino mass (\mneut).  In addition, I will utilize MaNGA IFU data to produce concrete galaxy property measurements and use models that couple galaxy observations with cosmological predictions to constrain how galaxies form and evolve their star formation.  With its impact on our understanding of cosmology and galaxy evolution, this research program directly addresses some of the main questions in the Cosmic Origins program.}\\ \vspace{-11mm}

\todo{talk about the future with eBOSS and DESI}
...
Each of the methodological improvements I propose above are doubly instrumental for extracting accurate and precise \mneut ~constraints from eBOSS and DESI.
...
However, the underlying drivers of the evolution are not directly revealed by observations. % and require 
...
The seminal MPA-JHU catalog\footnotemark[2] derived galaxy properties from optical spectra obtained from single-fibers~\citep{mpajhu_mass, mpajhu_sfr}. 
...
This ``\emph{galactic conformity}'' has been regarded as the ``smoking gun'' of the link between the formation history of halos and galaxy star formation.%~\citep[][]{hearin_conformity}. 
