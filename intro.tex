...
The perturbations that enter before the matter-radiation equality ($a(t) = a_{eq}$), 
during the radiation dominated era, have physical scales 
$\lambda_{phys} < \lambda_{eq} \propto a_{eq}$. These smaller scale perturbations, 
todo{explain the supression} 
...
P_{inf}(k) \prop k^{n_s}
\eeq
where $n_s \sim 1$ \todo{cite inflation papers}. Then powerspectrum of the density 
fluctuation in the late Universe can be expressed as 
\beq
P(k) \prop k^{n_s} T^2(k) D^2(k). 
\eeq
...
\beq
\delta_g({\bf r}) = \sum\limits_{k=0}^{\infty}\delta({\bf r} ). 
\eeq
