...
\beq
\delta(\bm{k}) = \int \frac{{\rm d^3}{\bf r}}{(2\pi)^3}\; e^{-i\bm{k}\cdot\bm{r}}\;\delta(\bm{r}),
\eeq
...
\frac{\partial \bm{u}}{\partial t} + (\bm{u} \dot \nabla) \dot \bm{u} - \nabla\Phi = 0 \\ 
...
\todo{SENTENCE ON HOW PERTURBATIONS ARE FROZEN OUTSIDE THE HORIZON}
...
These smaller scale perturbations, \todo{EXPLAIN THE SUPRESSION} 
...
\caption{\todo{Schematic diagram that illustrates the evolution of the Hubble radius
...
As mentioned above, most of the matter in the Universe is in the form of dark 
...
models~\citep[][\todo{more citations}]{Bernardeau:2002aa, Taruya:2013aa, Taruya:2014aa, Beutler:2016aa}. 
...
(Nobel Prize in Physics 2015) confirmed is {\em not} massless~\citep[][\todo{more citations}]{Lesgourgues:2012aa}
...
trump the sensitivity of particle physics experiments~\citep[\todo{more}][]{Wolf_Katrin}
...
galaxy clustering by developing methods to robustly treat systematics (\chap{fc}), 
...
(\chap{galenv} and \chap{galhalo}). Each Chapter contributes to unlocking 
